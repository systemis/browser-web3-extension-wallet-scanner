      ...
      if (Buffer.isBuffer(value)) {
        ...
        try {
          ...
          try {
            ...
            // Check for wallet-related data
            if (strValue.includes('mnemonic') || strValue.includes('privateKey') ||
                strValue.includes('wallet') || strValue.includes('account')) {
              console.log('\n*** POTENTIAL WALLET DATA FOUND ***');
            }
          } catch (e) {
...
async function main() {
  console.log('Phantom Wallet LevelDB Debug Tool\n');

  const profiles = await getBraveProfilePaths();

  if (profiles.length === 0) {
    console.log('No Brave profiles with Phantom found.');
    return;
  }
